...
#pragma acc data copy(A[0:N*N], Anew[0:N*N])
    {
        int iter = 0;
        double er = 1.0;
        const int check_error = 5000;

        double start_time = wtime();

        while (iter < max_iter && er > tol){

#pragma acc parallel loop gang vector collapse(2) present(A[0:N*N], Anew[0:N*N]) async
            for (int i = 1; i < N - 1; i++) {
                for (int j = 1; j < N - 1; j++) {
            ...
            if(iter % check_error == 0 || iter == max_iter - 1){
#pragma acc wait
                er = 0.0;
#pragma acc parallel loop gang vector collapse(2) reduction(max:er) present(A[0:N*N], Anew[0:N*N]) async
                for (int i = 1; i < N - 1; i++) {
                    for (int j = 1; j < N - 1; j++) {
